...
- [Node.js v24](https://nodejs.org/en/download/package-manager).
  Note that version 24 is _required_.
  We do not support older or newer versions at the moment.
    ...
    <details>
      ...
      <Tabs items={["Linux and MacOS", "Windows"]}>
        <Tab>
          ```sh copy
          curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash nvm install 24
          ```
        </Tab>
...
1. Create a new project from a template with pnpm.
   ...
   {/* MDX comment shields the loose list from prettier issue #17746 */}
