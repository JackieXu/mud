    ...
    const request: TransportRequestFn<BundlerRpcSchema> = async ({ method, params }) => {
      ...
      if (method === "eth_getUserOperationReceipt") {
        ...
        const userOpReceipt = getUserOperationReceipt(userOpHash, {
          ...
          blockTimestamp: receipt.blockTimestamp ? numberToHex(receipt.blockTimestamp) : undefined,
          ...
          logs: receipt.logs.map((log) => ({
            ...
            blockTimestamp: log.blockTimestamp ? numberToHex(log.blockTimestamp) : undefined,
